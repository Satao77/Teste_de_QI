...
function obrigatoria(nome) {
  ...
  if (!valor) {
    console.error(`[ERRO] Variável de ambiente obrigatória ausente: ${nome}`);
    process.exit(1);
  }
  return valor;
}
...
const config = {
  mpToken: obrigatoria("MP_ACCESS_TOKEN"),
  webhookSecret: process.env.MP_WEBHOOK_SECRET || "",
  baseUrl: obrigatoria("BASE_URL").replace(/\/+$/, ""),
...
const persistir = () => fs.writeFileSync(DB_PATH, JSON.stringify(db, null, 2));
